  ...
  try {
    ...
    if (!userId || !cvId || !skills || !Array.isArray(skills)) {
      return res.status(400).json({
        message: "Invalid input: userId, cvId, and skills array are required",
      });
    }
    ...
    for (const skill of skills) {
      if (!skill.category || !skill.items) {
        return res
          .status(400)
          .json({ message: "Each skill must have a category and items" });
      }
    }
...
export async function getSkills(req, res) {
  try {
    ...
    const skills = await Skills.findOne({ userId }).sort({ _id: -1 });
